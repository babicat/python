...
SITE = {
    'ko': {
        'menu': ['ABOUT', 'CONTACT', ],
        'about': { }, #~~~
        'main': {'url': '/ko', 'image_list': ['.main_block01_item', 'body > main > div.container > div.clearfix > div.main_block02 > ul > div > div > li.main_block02_list_item.slick-slide.slick-current.slick-active', '.col3_block_list_item', '.btn_block']},
    },
    'en': {
        'main': {'url': '/ekn', },
    },
...
class TestPokopangcom(BaseTestCase):
    ...
    def open_page(self, url):
        ...
        if 'treenod.com' not in url:
            self.assertTrue(result, msg=f'{url} 접속 불가 - FAIL')

        result = self.selenium.is_image_loaded()
        self.assertTrue(result, msg=f'{url} 이미지 로딩 실패 ')
        
        LOGGER.info('{0} 접속 - PASS'.format(url+lan))

    # Header 영역 확인
    def find_css(self, val):
        for key in self.Header[lan].keys():
            result = self.selenium.wait('css_selector', val)
    ...
    def find_banner(self, lan):
        if lan == 'en':
            ...
            self.Banner['Sticker_image_1'] = './div[@class_name="slick-track"]/div[1]/img'
    ...
    def check_scroll(self, lan, value):     # 스크롤 및 TOP 버튼으로 올라오기 - sona, conner 함수 참고하여 섞은 형태
        ...
        self.assertIsTrue(scroll_result <= 0, msg="{0} {1} 스크롤 불가 - FAIL".format(lan, value))

        self.button_click('#js-pagetop-link')
        
        result = self.selenium.wait('css_selector', '.logo')
    ...
    def new_move_page(self, menu_name):
        pass
        #el = find_menu_xpath('/header_menu/xxx/text == {menu_name}').click()
    ...
    def find_text(self, name, value, sentence):
        ...
        for node in list(result):
            content = node.text
            self.assertTrue(sentence not in content, msg='{0} 텍스트 미노출 - FAIL'.format(name))
...
# -------------------------------------페이지별 기능 검증 함수------------------------------------- #
    ...
    # 메인 화면 이동 및 노출 확인
    def check_main_page(self, site):
        self.open_page(MAIN_URL + site['url'])
        """
    ...
    def _other_2(self):
        for m in SITE[menu_list]:
            check_page(m)
    "

    def check_page(self, menu):
        self.move_page(menu['name'])
        self.find_text(menu['xxx', 'xxxxx', 'cccccon'])

        for node in menu['find_text']:
            ~~
    """
...
# -------------------------------------언어별 함수 작동 확인------------------------------------- #
    ...
    def test_TC01(self):
        for lang in CHECK_LANG:
            site = SITE[lang]
            self.check_main_page(site)
            #self.check_about_page(lang)
            """
